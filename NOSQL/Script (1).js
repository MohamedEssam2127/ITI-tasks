...
{firstName:1 , salary:1 , address:1 });
...
.forEach(ins=> print(`fullname :${ins.firstName } ${ins.lastName} , age :${ins.age}`));
...
//db.instructors.update({firstName:"mazen" , lastName:"mohammed"} ,{$set:{"courses.2":"jquery"}});
db.instructors.updateOne(
  { firstName: "mazen", lastName: "mohammed", courses: "EF" },{ $set: { "courses.$": "jquery" } });
...
db.instructors.updateOne(
  { firstName: "ahmed", lastName: "mohammed" },{ $unset: { courses: "" } });
